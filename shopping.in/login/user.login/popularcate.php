...
if(isset($_GET['action']))
{
	if($_GET['action']=="delete")
	{
		...
		$verification=mysql_query("SELECT * FROM ais_popular_cate WHERE id='$id'")or die(mysql_error().'verification');
		if(mysql_num_rows($verification)>=1)
		{
			$verification=mysql_fetch_array($verification);
			if(unlink('../popular_category/'.$verification['indexbanner']))
			{
				$deletion=mysql_query("DELETE FROM ais_popular_cate WHERE id='$id'")or die(mysql_error().'deletion');
				header("location:popularcate.php?msg=Successfully deleted");
			}
			else
			{
				$deletion=mysql_query("DELETE FROM ais_popular_cate WHERE id='$id'")or die(mysql_error().'deletion');
				header("location:popularcate.php?msg=error Occured");
			}
		}
		else
		{
			header("location:popularcate.php?msg=Banner not found");
		}
	}
}
if(isset($_POST['submission']))
{
	if(isset($_FILES['indexbanner']['tmp_name']))
	{
		...
		{
			if(move_uploaded_file($_FILES['indexbanner']['tmp_name'],'../popular_category/'.$_FILES['indexbanner']['name']))
			{
				$filename=$_FILES['indexbanner']['name'];
				$insertion=mysql_query("INSERT INTO ais_popular_cate VALUES ('','$filename')")or die(mysql_error());
				header("location:popularcate.php?msg=success");
			}
		}
		else
		{
			header("location:popularcate.php?msg=Only image allowed");
		}
	}
}
?>
...
<body class=" theme-blue">
    ...
    <script type="text/javascript">
		...
		function deleteBanner(id)
		{
			...
			if(ans)
			{
				window.location="popularcate.php?action=delete&id="+id;
			}
		}
    </script>
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
            ...
            <h1 class="page-title">Popular Category Banner</h1>
                    <ul class="breadcrumb">
            <li><a href="home.php">Home</a></li>
            <li class="active">Popular Category Banner</li>
        </ul>
        ...
        <div class="main-content">
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<table class="table">
					...
					<?php
						$image=mysql_query("SELECT * FROM ais_popular_cate")or die(mysql_error());
						if(mysql_num_rows($image)>=1)
						{
							...
							while($row=mysql_fetch_array($image))
							{
								echo '<tr>
										<td>'.$s_no.'</td>
										<td><img src="../popular_category/'.$row['image'].'" style="width:200px;height:160px;"></td>
