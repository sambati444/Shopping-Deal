        ...
        <div class="main-content">
			<table class="table">
				<tr>
					...
					<th width="10%">Status/Delete</th>
				</tr>
				<?php
					$productinfo=mysql_query("SELECT * FROM manage_product_master WHERE deleted='0'")or die(mysql_error());
					if(mysql_num_rows($productinfo)>=1)
					{
						...
						while($row=mysql_fetch_array($productinfo))
						{
							echo '<tr>
								...
								echo '<td width="25%">'.$row['product_name'].'</td>
								<td width="15%">'.$row['purchase_price'].'</td>
								<td width="15%">'.$row['sale_price'].'</td>
								<td width="10%">';
									if($row['status']=='0')
