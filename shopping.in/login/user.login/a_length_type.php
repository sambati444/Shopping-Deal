...
if(isset($_GET['action'])&&$_GET['action']=="status")
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_length_type WHERE id='$lengthid'")or die(mysql_error().'cant fetch tax info');
	if(mysql_num_rows($taxinfo)>=1)
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		if($taxinfo['status']==0)
		{
			$updation=mysql_query("UPDATE manage_length_type SET status='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		else
		{
			$updation=mysql_query("UPDATE manage_length_type SET status='0' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		header("location:a_length_type.php?msg=Length status changed");
	}
}
...
if(isset($_GET['action'])&&$_GET['action']=="delete")
{
	...
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		$updation=mysql_query("UPDATE manage_length_type SET deleted='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		header("location:a_length_type.php?msg=Length deleted successfully");
	}
}
...
if(isset($_POST['submission'])&&$_POST['submission']=='EDIT')
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_length_type WHERE deleted='0' AND name='$name' AND id!='$lengthid'")or die(mysql_error());
	if(mysql_num_rows($taxinfo)>=1)
	{
		header("location:a_length_type.php?msg=Length is already exist");
	}
	else
	{
		$updation=mysql_query("UPDATE manage_length_type SET name='$name',description='$desc' WHERE id='$lengthid'")or die(mysql_error().'updation');
		header("location:a_length_type.php?msg=Length updated successfully");
	}
}
/*CREATE NEW TAX*/
if(isset($_POST['submission'])&&$_POST['submission']=='CREATE')
{
	$name=mysql_real_escape_string(strip_tags($_POST['name']));
	$desc=mysql_real_escape_string(strip_tags($_POST['desc']));
	$taxinfo=mysql_query("SELECT * FROM manage_length_type WHERE deleted='0' AND name='$name'")or die(mysql_error());
	if(mysql_num_rows($taxinfo)>=1)
	{
		header("location:a_length_type.php?msg=Length is already exist");
	}
	else
	{
		...
		$lengthid=mysql_query("SELECT MAX(id) FROM manage_length_type")or die(mysql_error());
		$lengthid=mysql_fetch_array($lengthid);
		$lengthid=$lengthid[0]+1;
		$insertion=mysql_query("INSERT INTO manage_length_type VALUES('$lengthid','$name','$desc','1','0')")or die(mysql_error());
		header("location:a_length_type.php?msg=Length added successfully");
	}
}
?>
<!doctype html>
...
<body class=" theme-blue">
    ...
    <script type="text/javascript">
		...
		function actionForTax(action,id)
		{
			if(action=='status')
			{
				...
				if(ans)
				{
					window.location="a_length_type.php?id="+id+"&action=status";
				}
			}
			else if(action=='edit')
			{
				...
				if(ans)
				{
					window.location="a_length_type.php?id="+id+"&action=edit";
				}
			}
			else if(action=='delete')
			{
				...
				if(ans)
				{
					window.location="a_length_type.php?id="+id+"&action=delete";
				}
			}
		}
    </script>
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
            ...
            <h1 class="page-title">Length Type</h1>
                    <ul class="breadcrumb">
            <li><a href="home.php">Home</a></li>
            <li class="active">Manage Length Type</li>
        </ul>
        </div>
        <div class="main-content">
			...
			if(!isset($_GET['action']))
			{
				...
				<table class="table">
					<tr>
						...
						<th>Description</th>
						<td><input type="text" class="form-control" name="desc" autofocus required/></td>
					</tr>
					<tr>
			...
			{
				$lengthid=mysql_real_escape_string(strip_tags($_GET['id']));
				$taxinfo=mysql_query("SELECT * FROM manage_length_type WHERE id='$lengthid'");
				if(mysql_num_rows($taxinfo)>=1)
				{
					$taxinfo=mysql_fetch_array($taxinfo);
				}
				...
				{
					echo '<script>window.location="a_length_type.php?msg=Bad request";</script>';
				}
				echo '<form action="" method="post">
				<table class="table">
					<tr>
						...
						<th>Description</th>
						<td><input type="text" class="form-control" name="desc" value="'.$taxinfo['description'].'" autofocus required/></td>
					</tr>
					<tr>
			...
			<table id="myTable" class="display table">
				...
				<tbody>
				<?php
				$taxtype=mysql_query("SELECT * FROM manage_length_type WHERE deleted='0'")or die(mysql_error());
