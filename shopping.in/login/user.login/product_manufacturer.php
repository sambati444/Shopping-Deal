...
if(isset($_GET['action'])&&$_GET['action']=="status")
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_manufacturer_master WHERE id='$lengthid'")or die(mysql_error().'cant fetch tax info');
	if(mysql_num_rows($taxinfo)>=1)
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		if($taxinfo['status']==0)
		{
			$updation=mysql_query("UPDATE manage_manufacturer_master SET status='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		else
		{
			$updation=mysql_query("UPDATE manage_manufacturer_master SET status='0' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		header("location:product_manufacturer.php?msg=Manufacturer status changed");
	}
}
...
if(isset($_GET['action'])&&$_GET['action']=="delete")
{
	...
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		$updation=mysql_query("UPDATE manage_manufacturer_master SET deleted='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		header("location:product_manufacturer.php?msg=Manufacturer deleted successfully");
	}
}
...
if(isset($_POST['submission'])&&$_POST['submission']=='EDIT')
{
	...
	if(isset($_FILES['imgfile']['tmp_name']))
	{
		...
		if(move_uploaded_file($_FILES['imgfile']['tmp_name'],'../product_manufacturer/'.$imgfile))
		{
			$taxinfo=mysql_query("SELECT * FROM manage_manufacturer_master WHERE deleted='0' AND name='$name' AND id!='$lengthid'")or die(mysql_error());
			if(mysql_num_rows($taxinfo)>=1)
			{
				header("location:product_manufacturer.php?msg=Manufacturer is already exist");
			}
			else
			{
				$updation=mysql_query("UPDATE manage_manufacturer_master SET name='$name',description='$desc',imagepath='$imgfile' WHERE id='$lengthid'")or die(mysql_error().'updation');
				header("location:product_manufacturer.php?msg=Manufacturer updated successfully");
			}
		}
		else
		{
			...
			{
				$updation=mysql_query("UPDATE manage_manufacturer_master SET name='$name',description='$desc' WHERE id='$lengthid'")or die(mysql_error().'updation');
				header("location:product_manufacturer.php?msg=Manufacturer updated successfully");
			}
		}
	}
...
if(isset($_POST['submission'])&&$_POST['submission']=='CREATE')
{
	...
	if(isset($_FILES['imgfile']['tmp_name']))
	{
		...
		if(move_uploaded_file($_FILES['imgfile']['tmp_name'],'../product_manufacturer/'.$imgfile))
		{
			$taxinfo=mysql_query("SELECT * FROM manage_manufacturer_master WHERE deleted='0' AND name='$name'")or die(mysql_error());
			if(mysql_num_rows($taxinfo)>=1)
			{
				header("location:product_manufacturer.php?msg=Manufacturer is already exist");
			}
			else
			{
				...
				$lengthid=mysql_query("SELECT MAX(id) FROM manage_manufacturer_master")or die(mysql_error());
				$lengthid=mysql_fetch_array($lengthid);
				$lengthid=$lengthid[0]+1;
				$insertion=mysql_query("INSERT INTO manage_manufacturer_master VALUES('$lengthid','$name','$desc','$imgfile','1','0')")or die(mysql_error());
				header("location:product_manufacturer.php?msg=Manufacturer added successfully");
			}
		}
		else
		{
			...
			else
			{
				...
				$lengthid=mysql_fetch_array($lengthid);
				$lengthid=$lengthid[0]+1;
				$insertion=mysql_query("INSERT INTO manage_manufacturer_master VALUES('$lengthid','$name','$desc','','1','0')")or die(mysql_error());
				header("location:product_manufacturer.php?msg=Manufacturer added successfully");
			}
		}
	}
    ...
    <script type="text/javascript">
		...
		function actionForTax(action,id)
		{
			if(action=='status')
			{
				...
				if(ans)
				{
					window.location="product_manufacturer.php?id="+id+"&action=status";
				}
			}
			else if(action=='edit')
			{
				...
				if(ans)
				{
					window.location="product_manufacturer.php?id="+id+"&action=edit";
				}
			}
			else if(action=='delete')
			{
				...
				if(ans)
				{
					window.location="product_manufacturer.php?id="+id+"&action=delete";
				}
			}
		}
    </script>
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
        ...
        <div class="main-content">
			...
			{
				$lengthid=mysql_real_escape_string(strip_tags($_GET['id']));
				$taxinfo=mysql_query("SELECT * FROM manage_manufacturer_master WHERE id='$lengthid'");
				if(mysql_num_rows($taxinfo)>=1)
				{
					$taxinfo=mysql_fetch_array($taxinfo);
				}
				...
				{
					echo '<script>window.location="product_manufacturer.php?msg=Bad request";</script>';
				}
				echo '<form action="" method="post" enctype="multipart/form-data">
				<table class="table">
					<tr>
			...
			<table id="myTable" class="display table">
				...
				<tbody>
				<?php
				$taxtype=mysql_query("SELECT * FROM manage_manufacturer_master WHERE deleted='0'")or die(mysql_error());
				if(mysql_num_rows($taxtype)>=1)
				{
					...
					while($taxrow=mysql_fetch_array($taxtype))
					{
						echo '<tr>
							<td>'.$s_no.'</td>';
							if($taxrow['imagepath']!='')
							echo '<td><img src="../product_manufacturer/'.$taxrow['imagepath'].'" class="m_logo"></td>';
							else
							echo '<td>&nbsp;</td>';
