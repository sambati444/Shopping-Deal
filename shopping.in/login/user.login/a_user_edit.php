...
if(isset($_POST['submission']))
{
	...
	$userinfo=mysql_query("SELECT * FROM ais_users WHERE id!='$userid' AND username='$username'")or die(mysql_error());
	if(mysql_num_rows($userinfo)>=1)
	{
		header("location:a_user_edit.php?msg=Username is already registered!!!&id=".$userid);
	}
	else
	{
		if(!empty($_POST['password']))
		{
			$updation=mysql_query("UPDATE ais_users SET username='$username',password='$password',name='$name',fathername='$fathername',contactno='$contactno',emailid='$emailid',dateOfJoining='$dateofjoining',designation='$designation',residential_address='$resi_address',permanent_address='$perm_address',city='$city',state='$state',country='$country',pincode='$pincode' WHERE id='$userid'")or die(mysql_error());
			header("location:a_user_view.php?msg=User updated successfully");
		}
		else
		{
			$updation=mysql_query("UPDATE ais_users SET username='$username',name='$name',fathername='$fathername',contactno='$contactno',emailid='$emailid',dateOfJoining='$dateofjoining',designation='$designation',residential_address='$resi_address',permanent_address='$perm_address',city='$city',state='$state',country='$country',pincode='$pincode' WHERE id='$userid'")or die(mysql_error());
			header("location:a_user_view.php?msg=User updated successfully");
		}
	}
}
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
        ...
        <div class="main-content">
			...
			<table class="table">
				<tr>
					<th>Username</th>
					<td><input type="text" class="form-control" name="username" value="<?php echo $userinfo['username'];?>" autofocus required/></td>
					<th>Password</th>
					<td><input type="text" class="form-control" name="password" ></td>
				</tr>
				<tr>
					<th>Name</th>
					<td><input type="text" class="form-control" name="name" value="<?php echo $userinfo['name'];?>" autofocus required/></td>
					<th>Father Name</th>
					<td><input type="text" class="form-control" name="fathername" value="<?php echo $userinfo['fathername'];?>" autofocus required/></td>
				</tr>
				<tr>
					<th>Contact No.</th>
					<td><input type="text" class="form-control" name="contactno" value="<?php echo $userinfo['contactno'];?>" autofocus required/></td>
					<th>E-Mail ID</th>
					<td><input type="email" class="form-control" name="emailid" value="<?php echo $userinfo['emailid'];?>" autofocus required/></td>
				</tr>
				<tr>
					<th>Designation</th>
					<td>
						<select class="form-control" name="designation" autofocus required>
							...
							if(mysql_num_rows($pre_designation)>=1)
							{
								...
								{
									if($designation['designation']==$userinfo['designation'])
									{
										echo '<option value="'.$designation['designation'].'" selected/>'.$designation['designation'].'</option>';
									}
									else
									{
					...
					<td><input type="date" class="form-control" name="dateofjoining" value="<?php echo $userinfo['dateOfJoining'];?>" autofocus required/></td>
				</tr>
				<tr>
					<th>Residential Address</th>
					<td colspan="3"><textarea class="form-control" name="resi_address" id="resi_address" autofocus required/><?php echo $userinfo['residential_address'];?></textarea></td>
				</tr>
				<tr>
					<th>Permanent Address</th>
					<td colspan="3">
						<input type="checkbox" id="sameasresiadd" onclick="sameAsResiAdd()">Same As Residential Address
						<textarea class="form-control" name="perm_address" id="perm_address" autofocus required/><?php echo $userinfo['permanent_address'];?></textarea>
					</td>
				</tr>
				<tr>
					<th>Country</th>
						...
						<select class="form-control" name="country" id="country" onchange="selectState(this.value)" autofocus required/>
							...
							<?php
								...
								if(mysql_num_rows($countryinfo)>=1)
								{
									...
									{
										if($userinfo['country']==$row['country'])
					...
					<td>
						<select class="form-control" name="state" id="state" onchange="selectCity(this.value)" autofocus required/>
							<?php echo '<option value="'.$userinfo['state'].'">'.$userinfo['state'].'</option>';?>
							<!--DYNAMIC CONTENT-->
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select class="form-control" name="city" id="city" autofocus required/>
							<?php echo '<option value="'.$userinfo['city'].'">'.$userinfo['city'].'</option>';?>
							<!--DYNAMIC CONTENT-->
						</select>
					</td>
					<th>Pincode</th>
