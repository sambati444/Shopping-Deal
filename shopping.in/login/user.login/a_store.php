...
if(isset($_POST['submission']))
{
	...
	$storeid=mysql_query("SELECT MAX(id) FROM store_master")or die(mysql_error());
	$storeid=mysql_fetch_array($storeid);
	$storeid=$storeid[0]+1;
	$username=$email;
	$password=md5($mobile);
	$query=mysql_query("INSERT INTO store_master VALUES('$storeid','$username','$password','$storename','$owner','$address','$contact','$mobile','$email','0','0')")or die(mysql_error());
        ...
        <div class="main-content">
			...
			<form action="" method="post" enctype="multipart/form-data">
				<table class="table">
					<tr>
						<th>Store Name</th>
						<td colspan="3"><input type="text" name="storename" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Owner Name</th>
						<td><input type="text" name="owner" class="form-control" autofocus required/></td>
						<th>Contact No.</th>
						<td><input type="number" name="contact" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Mobile No.</th>
						<td><input type="number" name="mobile" class="form-control" autofocus required/></td>
						<th>E-Mail ID</th>
						<td><input type="email" name="email" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Address</th>
			...
			<table id="myTable" class="display table">
				...
				<tbody>
				<?php
				$store=mysql_query("SELECT * FROM store_master WHERE deleted='0'")or die(mysql_error());
				if(mysql_num_rows($store)>=1)
				{
					$s_no=1;
					while($row=mysql_fetch_array($store))
					{
						echo '<tr>
							<td>'.$s_no.'</td>
							<td>'.$row['storename'].'</td>
							...
							<td>';
							if($row['status']==0)
							echo '<a href="a_store_status.php?id='.$row['id'].'" class="btn" style="background:#FF0000;color:#FFFFFF;">Inactive</a>';
							else
							echo '<a href="a_store_status.php?id='.$row['id'].'" class="btn" style="background:#08C000;color:#FFFFFF;">Active</a>';
							echo '&nbsp;<a href="a_store_edit.php?id='.$row['id'].'" class="btn btn-info">Edit</a>';
							echo '&nbsp;<a href="a_store_delete.php?id='.$row['id'].'" class="btn btn-danger">Delete</a>';
							echo '</td>
						</tr>';
						$s_no=$s_no+1;
					}
