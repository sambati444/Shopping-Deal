...
if(isset($_GET['action'])&&$_GET['action']=="status")
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_menu WHERE id='$lengthid'")or die(mysql_error().'cant fetch tax info');
	if(mysql_num_rows($taxinfo)>=1)
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		if($taxinfo['status']==0)
		{
			$updation=mysql_query("UPDATE manage_menu SET status='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		else
		{
			$updation=mysql_query("UPDATE manage_menu SET status='0' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		header("location:product_webmenu.php?msg=Menu status changed");
	}
}
/*DELETE*/
if(isset($_GET['action'])&&$_GET['action']=="delete")
{
	...
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		$updation=mysql_query("UPDATE manage_menu SET deleted='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		header("location:product_webmenu.php?msg=Menu deleted successfully");
	}
}
/*EDIT*/
if(isset($_POST['submission'])&&$_POST['submission']=='EDIT')
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_menu WHERE deleted='0' AND name='$name' AND id!='$lengthid'")or die(mysql_error());
	if(mysql_num_rows($taxinfo)>=1)
	{
		header("location:product_webmenu.php?msg=Length is already exist");
	}
	else
	{
		$updation=mysql_query("UPDATE manage_menu SET name='$name' WHERE id='$lengthid'")or die(mysql_error().'updation');
		header("location:product_webmenu.php?msg=Menu updated successfully");
	}
}
/*CREATE*/
...
{
	$name=mysql_real_escape_string(strip_tags($_POST['name']));
	$taxinfo=mysql_query("SELECT * FROM manage_menu WHERE deleted='0' AND name='$name'")or die(mysql_error());
	if(mysql_num_rows($taxinfo)>=1)
	{
		header("location:product_webmenu.php?msg=Menu is already exist");
	}
	else
	{
		/*INSERTION QUERY FOR TAX*/
		$lengthid=mysql_query("SELECT MAX(id) FROM manage_menu")or die(mysql_error());
		$lengthid=mysql_fetch_array($lengthid);
		$lengthid=$lengthid[0]+1;
		$insertion=mysql_query("INSERT INTO manage_menu VALUES('$lengthid','$name','1','0')")or die(mysql_error());
		header("location:product_webmenu.php?msg=Menu added successfully");
	}
}
?>
<!doctype html>
...
<body class=" theme-blue">
    ...
    <script type="text/javascript">
		...
		function actionForTax(action,id)
		{
			if(action=='status')
			{
				...
				if(ans)
				{
					window.location="product_webmenu.php?id="+id+"&action=status";
				}
			}
			else if(action=='edit')
			{
				...
				if(ans)
				{
					window.location="product_webmenu.php?id="+id+"&action=edit";
				}
			}
			else if(action=='delete')
			{
				...
				if(ans)
				{
					window.location="product_webmenu.php?id="+id+"&action=delete";
				}
			}
		}
    </script>
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
            ...
            <li class="active">Manage Web Menus</li>
        </ul>
        </div>
        <div class="main-content">
			...
			{
				$lengthid=mysql_real_escape_string(strip_tags($_GET['id']));
				$taxinfo=mysql_query("SELECT * FROM manage_menu WHERE id='$lengthid'");
				if(mysql_num_rows($taxinfo)>=1)
				{
					$taxinfo=mysql_fetch_array($taxinfo);
				}
				...
				{
					echo '<script>window.location="product_webmenu.php?msg=Bad request";</script>';
				}
				echo '<form action="" method="post">
				<table class="table">
					<tr>
			...
			<table class="table">
				...
				<?php
				$taxtype=mysql_query("SELECT * FROM manage_menu WHERE deleted='0'")or die(mysql_error());
