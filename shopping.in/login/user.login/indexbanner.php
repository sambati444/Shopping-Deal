...
if(isset($_GET['action']))
{
	if($_GET['action']=="delete")
	{
		...
		$verification=mysql_query("SELECT * FROM ais_indexbanner WHERE id='$id'")or die(mysql_error().'verification');
		if(mysql_num_rows($verification)>=1)
		{
			$verification=mysql_fetch_array($verification);
			if(unlink('../setting/images/indexbanner/'.$verification['indexbanner']))
			{
				$deletion=mysql_query("DELETE FROM ais_indexbanner WHERE id='$id'")or die(mysql_error().'deletion');
				header("location:indexbanner.php?msg=Successfully deleted");
			}
			else
			{
				$deletion=mysql_query("DELETE FROM ais_indexbanner WHERE id='$id'")or die(mysql_error().'deletion');
				header("location:indexbanner.php?msg=error Occured");
			}
		}
		else
		{
			header("location:indexbanner.php?msg=Banner not found");
		}
	}
}
if(isset($_POST['submission']))
{
	if(isset($_FILES['indexbanner']['tmp_name']))
	{
		...
		{
			if(move_uploaded_file($_FILES['indexbanner']['tmp_name'],'../setting/images/indexbanner/'.$_FILES['indexbanner']['name']))
			{
				$filename=$_FILES['indexbanner']['name'];
				$insertion=mysql_query("INSERT INTO ais_indexbanner VALUES ('','$filename')")or die(mysql_error());
				header("location:indexbanner.php?msg=success");
			}
		}
		else
		{
			header("location:indexbanner.php?msg=Only image allowed");
		}
	}
}
?>
...
<body class=" theme-blue">
    ...
    <script type="text/javascript">
		...
		function deleteBanner(id)
		{
			...
			if(ans)
			{
				window.location="indexbanner.php?action=delete&id="+id;
			}
		}
    </script>
        ...
        <div class="main-content">
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<table class="table">
					...
					<?php
						$image=mysql_query("SELECT * FROM ais_indexbanner")or die(mysql_error());
						if(mysql_num_rows($image)>=1)
						{
							...
							while($row=mysql_fetch_array($image))
							{
								echo '<tr>
										<td>'.$s_no.'</td>
										<td><img src="../setting/images/indexbanner/'.$row['indexbanner'].'" style="width:400px;height:260px;"></td>
