...
$customerinfo=mysql_query("SELECT * FROM customer_master WHERE id='$id'")or die(mysql_error());
...
if(isset($_POST['submission']))
{
	...
	if(mysql_num_rows($customerinfo)>=1)
	{
		header("location:customer_edit.php?msg=E-mail ID already registered");
	}
	else
	{
		$insertion=mysql_query("UPDATE customer_master SET fname='$fname',lname='$lname',username='$username',emailid='$emailid',mobileno='$mobileno',landlineno='$landlineno',faxno='$faxno',residential_address='$address',city='$city',state='$state',pincode='$pincode',country='$country' WHERE id='$id'")or die(mysql_error().'insertion');
		header("location:customerinfo.php?msg=success");
	}
}
?>
<!doctype html>
...
<!--LOADER AJAX-->
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
            ...
            <li class="active">Edit Exist Customer</li>
        </ul>
			...
			<form action="" method="post" enctype="multipart/form-data">
				<table class="table">
					<tr>
						...
						<td colspan="3"><input type="text" name="friend" class="form-control" value="<?php echo $customerinfo['friend_reference'];?>" readonly/></td>
					</tr>
					<tr>
						<th>First Name</th>
						<td><input type="text" name="fname" class="form-control" value="<?php echo $customerinfo['fname'];?>" autofocus required/></td>
						<th>Last Name</th>
						<td><input type="text" name="lname" class="form-control" value="<?php echo $customerinfo['lname'];?>" autofocus required/></td>
					</tr>
					<tr>
						<th>Mobile No.</th>
						<td><input type="number" name="mobileno" class="form-control" value="<?php echo $customerinfo['mobileno'];?>" autofocus required/></td>
						<th>E-Mail ID</th>
						<td><input type="email" name="emailid" class="form-control" value="<?php echo $customerinfo['emailid'];?>" autofocus required/></td>
					</tr>
					<tr>
						<th>Landline No.</th>
						<td><input type="number" name="landlineno" class="form-control" value="<?php echo $customerinfo['landlineno'];?>"></td>
						<th>Fax No.</th>
						<td><input type="number" name="faxno" class="form-control" value="<?php echo $customerinfo['faxno'];?>"></td>
					</tr>
					<tr>
						<th>Address</th>
						<td colspan="3"><textarea name="address" class="form-control" autofocus required/><?php echo $customerinfo['residential_address'];?></textarea></td>
					</tr>
					<tr>
						<th>Country</th>
						<td>
							<select name="country" id="country" class="form-control" onchange="selectState(this.value)" autofocus required/>
								...
								<?php
									...
									if(mysql_num_rows($country)>=1)
									{
										...
										{
											if($customerinfo['country']==$row['country'])
						...
						<td>
							<select name="state" id="state" class="form-control" onchange="selectCity(this.value)" autofocus required/>
								<?php echo '<option value="'.$customerinfo['state'].'">'.$customerinfo['state'].'</option>';?>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<select name="city" id="city" class="form-control" autofocus required/>
								<?php echo '<option value="'.$customerinfo['city'].'">'.$customerinfo['city'].'</option>';?>
							</select>
						</td>
						<th>Pincode</th>
						<td><input type="number" name="pincode" id="pincode" class="form-control" value="<?php echo $customerinfo['pincode'];?>" autofocus required/></td>
					</tr>
					<tr>
