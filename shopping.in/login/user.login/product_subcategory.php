...
if(isset($_GET['action'])&&$_GET['action']=="status")
{
	...
	$taxinfo=mysql_query("SELECT * FROM manage_subcategory WHERE id='$lengthid'")or die(mysql_error().'cant fetch tax info');
	if(mysql_num_rows($taxinfo)>=1)
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		if($taxinfo['status']==0)
		{
			$updation=mysql_query("UPDATE manage_subcategory SET status='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		else
		{
			$updation=mysql_query("UPDATE manage_subcategory SET status='0' WHERE id='$lengthid'")or die(mysql_error().'1');
		}
		header("location:product_subcategory.php?msg=Sub-category status changed");
	}
}
/*DELETE*/
if(isset($_GET['action'])&&$_GET['action']=="delete")
{
	...
	{
		$taxinfo=mysql_fetch_array($taxinfo);
		$updation=mysql_query("UPDATE manage_subcategory SET deleted='1' WHERE id='$lengthid'")or die(mysql_error().'1');
		header("location:product_subcategory.php?msg=Sub-category deleted successfully");
	}
}
/*EDIT*/
if(isset($_POST['submission'])&&$_POST['submission']=='EDIT')
{
	...
	$categoryid=mysql_real_escape_string(strip_tags($_POST['categoryid']));
	if(isset($_FILES['banner']['tmp_name']))
	{
		$bannerpath=addslashes('../subcategory/'.$_FILES['banner']['name']);
		$banner=$_FILES['banner']['name'];
		if(move_uploaded_file($_FILES['banner']['tmp_name'],$bannerpath))
		{
			$taxinfo=mysql_query("SELECT * FROM manage_subcategory WHERE deleted='0' AND category_id='$categoryid' AND name='$name' AND id!='$lengthid'")or die(mysql_error());
			if(mysql_num_rows($taxinfo)>=1)
			{
				header("location:product_subcategory.php?msg=Sub-category is already exist");
			}
			else
			{
				$updation=mysql_query("UPDATE manage_subcategory SET name='$name',category_id='$categoryid',imagepath='$banner' WHERE id='$lengthid'")or die(mysql_error().'updation');
				header("location:product_subcategory.php?msg=Sub Category updated successfully");
			}
		}
		else
		{
			...
			{
				$updation=mysql_query("UPDATE manage_subcategory SET name='$name',category_id='$categoryid' WHERE id='$lengthid'")or die(mysql_error().'updation');
				header("location:product_subcategory.php?msg=Category updated successfully");
			}
		}
	}
	else
...
if(isset($_POST['submission'])&&$_POST['submission']=='CREATE')
{
	...
	if(isset($_FILES['banner']['tmp_name']))
	{
		...
		if(move_uploaded_file($_FILES['banner']['tmp_name'],$bannerpath))
		{
			$taxinfo=mysql_query("SELECT * FROM manage_subcategory WHERE deleted='0' AND category_id='$categoryid' AND name='$name'")or die(mysql_error());
			if(mysql_num_rows($taxinfo)>=1)
			{
				header("location:product_subcategory.php?msg=Sub Category is already exist");
			}
			else
			{
				...
				$lengthid=mysql_query("SELECT MAX(id) FROM manage_subcategory")or die(mysql_error());
				$lengthid=mysql_fetch_array($lengthid);
				$lengthid=$lengthid[0]+1;
				$insertion=mysql_query("INSERT INTO manage_subcategory VALUES('$lengthid','$name','$categoryid','$banner','1','0')")or die(mysql_error());
				header("location:product_subcategory.php?msg=Sub Category added successfully");
			}
		}
		else
		{
			...
			else
			{
				...
				$lengthid=mysql_fetch_array($lengthid);
				$lengthid=$lengthid[0]+1;
				$insertion=mysql_query("INSERT INTO manage_subcategory VALUES('$lengthid','$name','$categoryid','','1','0')")or die(mysql_error());
				header("location:product_subcategory.php?msg=Category added successfully");
			}
		}
	}
	else
    ...
    <script type="text/javascript">
		...
		function actionForTax(action,id)
		{
			if(action=='status')
			{
				...
				if(ans)
				{
					window.location="product_subcategory.php?id="+id+"&action=status";
				}
			}
			else if(action=='edit')
			{
				...
				if(ans)
				{
					window.location="product_subcategory.php?id="+id+"&action=edit";
				}
			}
			else if(action=='delete')
			{
				...
				if(ans)
				{
					window.location="product_subcategory.php?id="+id+"&action=delete";
				}
			}
		}
    </script>
  ...
  <!--<![endif]-->
    ...
    <p class="stat"><span class="label label-danger">15</span> Overdue</p>-->
        ...
        <div class="main-content">
			...
			{
				$lengthid=mysql_real_escape_string(strip_tags($_GET['id']));
				$taxinfo=mysql_query("SELECT * FROM manage_subcategory WHERE id='$lengthid'");
				if(mysql_num_rows($taxinfo)>=1)
				{
					$taxinfo=mysql_fetch_array($taxinfo);
				}
				...
				{
					echo '<script>window.location="product_subcategory.php?msg=Bad request";</script>';
				}
				echo '<form action="" method="post" enctype="multipart/form-data">
				<table class="table">
					<tr>
			...
			<table id="myTable" class="display table">
				...
				<tbody>
				<?php
				$taxtype=mysql_query("SELECT * FROM manage_subcategory WHERE deleted='0'")or die(mysql_error());
				if(mysql_num_rows($taxtype)>=1)
				{
					...
					while($taxrow=mysql_fetch_array($taxtype))
					{
						echo '<tr>
							<td>'.$s_no.'</td>
							<td>'.$taxrow['name'].'</td>';
							$categoryinfo=mysql_query("SELECT * FROM manage_category WHERE id='$taxrow[category_id]'")or die(mysql_error().'fetch data');
							$categoryinfo=mysql_fetch_array($categoryinfo);
							echo '<td>'.$categoryinfo['name'].'</td>';
							echo '<td><img src="../subcategory/'.$taxrow['imagepath'].'" style="width:200px;height:160px;"></td>
							<td>';
							if($taxrow['status']==0)
							{
