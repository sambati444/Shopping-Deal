...
if(isset($_POST['submission']))
{
	...
	{
		$productid=mysql_query("SELECT MAX(id) FROM manage_product_master")or die(mysql_error());
		$productid=mysql_fetch_array($productid);
		$productid=$productid[0]+1;
		$photo1=$productid.'_1.jpg';
		$photo2=$productid.'_2.jpg';
		$photo3=$productid.'_3.jpg';
		$photo4=$productid.'_4.jpg';
		if(!move_uploaded_file($_FILES['photo1']['tmp_name'],'../product/'.$photo1))
		{
			$photo1='';
		}
		if(!move_uploaded_file($_FILES['photo2']['tmp_name'],'../product/'.$photo2))
		{
			$photo2='';
		}
		if(!move_uploaded_file($_FILES['photo3']['tmp_name'],'../product/'.$photo3))
		{
			$photo3='';
		}
		if(!move_uploaded_file($_FILES['photo4']['tmp_name'],'../product/'.$photo4))
		{
			$photo4='';
		}
		$query=mysql_query("INSERT INTO manage_product_master VALUES('$productid','$_SESSION[storeid]','$category','$subcategory','$modelno','$productname','$manufacturer','$marketer','$quantity','$shippingprice','$saleprice','$oldprice','$tax','$purchaseprice','$weight','$weighttype','$width','$height','$lengthtype','$photo1','$photo2','$photo3','$photo4','$description','$dateofreg','$dateofreg','$special','0','0','0')")or die(mysql_error());
		header("location:product_add.php?msg=success&trigger=1");
	}
	else
	{
		$productid=mysql_query("SELECT MAX(id) FROM manage_product_master")or die(mysql_error());
		...
		$photo4='';
		$query=mysql_query("INSERT INTO manage_product_master VALUES('$productid','$_SESSION[storeid]','$category','$subcategory','$modelno','$productname','$manufacturer','$marketer','$quantity','$shippingprice','$saleprice','$oldprice','$tax','$purchaseprice','$weight','$weighttype','$width','$height','$lengthtype','$photo1','$photo2','$photo3','$photo4','$description','$dateofreg','$dateofreg','$special','0','0','0')")or die(mysql_error());
		header("location:product_add.php?msg=success&trigger=2");
	}
}
?>
<!doctype html>
        ...
        <div class="main-content">
			...
			<form action="" method="post" enctype="multipart/form-data">
				<table class="table">
					...
					<tr>
						<th>Product Name</th>
						<td colspan="3"><input type="text" name="productname" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Model No.</th>
						<td><input type="text" name="modelno" class="form-control" autofocus required/></td>
						<th>Quantity</th>
						<td><input type="number" name="quantity" class="form-control" autofocus required/></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="text" name="oldprice" class="form-control" autofocus required/></td>
						<th>Sale Price</th>
						<td><input type="text" name="saleprice" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Shipping Price</th>
						<td><input type="text" name="shippingprice" class="form-control" autofocus required/></td>
						<th>Tax</th>
						<td>
					...
					<tr>
						...
						<td><input type="text" name="purchaseprice" class="form-control" autofocus required/></td>
						<th>Weight</th>
						<td><input type="text" name="weight" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Weight type</th>
						<td>
						...
						<th>Width</th>
						<td><input type="text" name="width" class="form-control" autofocus required/></td>
					</tr>
					<tr>
						<th>Height</th>
						<td><input type="text" name="height" class="form-control" autofocus required/></td>
						<th>Length type</th>
						<td>
					...
					<tr>
						<td colspan="4"><input type="submit" name="submission" class="btn btn-info pull-right" value="CREATE PRODUCT"></td>
					</tr>
				</table>
			</form>
		</div>
